...
const forgotPassword = async (req, res) => {
  try {
    const { email: email, firstname: firstname } = req.body;
    ...
    if (!user) {
      return res
        .status(404)
        .send("Aucun utilisateur trouvé avec cette adresse e-mail.");
    }

    // 2. Générer un token JWT
    const token = jwt.sign({ id: user.userId }, SECRET, { expiresIn: "1h" });

    // Créer un objet de transport SMTP avec la configuration de votre serveur
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <body style="margin: 0; padding: 0; box-sizing: border-box; background-color: #ECECEC; align="center"">
    ...
    return res.status(500).send(`Erreur du serveur: ${error.message}`);
  }
};
